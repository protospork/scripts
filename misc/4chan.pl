...
my ($rename, $numtries, $thread, $numbering, $immortal, $timeout, $threadtitle, $md5filenames, $nevershorten, $takeyourtime, $imagelimit) = (0,999999,0,1,0,86400,0,0,0,0,0);
for (@ARGV){
	if ($_ =~ s/^-//){	#option switches
		if ($_ =~ /(.*?)r/){ $rename = 1 unless $1 =~ /:/; }		#'r'enames files to their 'original' names	#this might as well just be the default
		...
		if ($_ =~ /t:(\d+)/){ $timeout = $1*60; }					#'t'imeout (in minutes) before the script gives up on an inactive thread	#I'm not sure why this exists
		if ($_ =~ /n:(\w+)/){ $threadtitle = $1 }					#'n'ame of thread [default is thread num]
		if ($_ =~ /(.*?)b/){ $numbering = 0 unless $1 =~ /:/; }		#'b'are, no added numbering	#THIS MODE MUST BE USED ON /B/
		...
		#else { exit print "invalid argument: $_"; }
	} elsif ($_ =~ /^http/){
		$thread = $_;
	} else {
...
#$thread = $ARGV[-1];	#why?
if ($thread eq 0){ exit print "don't see a URL" }	#but isn't this... wha?

my ($count,$lastDL,$firstscrape,$offline) = (time,1,1,0);
while ($numtries > 0){
	body();	#was it really that easy!?
	$numtries--; $firstscrape = 0;
	unless ($numtries == 0){ say $numtries." tries remaining"; } else { say "Done."; done(); }
	
	my @now = localtime();
	
	if ($count == 127 && $thread =~ m{/(s|hr)/}){ $imagelimit = 1; }
	if ($offline == 1){ sleep 3600; }
	elsif ((time - $lastDL) > $timeout && $lastDL != 1 && $immortal == 0){ say "reached timeout length without any new images posted"; done(); }
	elsif (($now[2] < 2 || $now[2] > 7) && $takeyourtime == 1){ sleep 1800; }
	else { sleep 600; }
}
sub body {
	chdir $root_dir;
	...
	my $albumname = $thread;
	$albumname =~ s/^.+(?:org|com|net|info)\///i; $albumname =~ s/\/res//i; #remove everything except "[boardname]/[threadnumber]"
	$albumname =~ s[/.+$][/$threadtitle] unless $threadtitle eq '0';
	say $albumname;	make_path($albumname);
	chdir $albumname; 
	
	my $html = $page->content; $html =~ s/<script.+?\/script>//gis;	#remove the captcha and junk	
	
	my @junk = $html =~ m{(<span class="filesize">File : .+?src=http.+?name=".+?class="filetitle">.+?class="postername".+?<span id=".+?class="quotejs">.+?</blockquote>|class="quotejs">[\dx]+</a>.+?<span class="filesize">.+?</blockquote>)}gsi; #fuckin /b/ with those Xes
...
#	open TEST, '>', 'test.txt';	print TEST (join "\n\n", @junk); close TEST;
	...
	$count = 0;
	for my $trash (@junk){
		my ($link) = $trash =~ m{href="(http://\S+/src/\d+\.(?:jpe?g|gif|png))"}si; if ($link eq ''){ exit print "regex fail 116"; }
		push @URLs, $link;
		if ($rename == 1){	#conditionally restore original names to images
			($realname{$link}) = $trash =~ m{title="([^"]+?)"}si;
			$realname{$link} =~ s/\.+/./g; $realname{$link} =~ s/ +/_/g; $realname{$link} =~ s/\?/_/g;
		} else {
		...
		($postnum{$link}) = $trash =~ m{"quotejs">\d+(\d\d\d\d\d)(?:XXX)?</a}si;	#hopefully five numbers plus the filename itself can avoid collisions
...
#			$realname{$link} = ((sprintf "%03d", $count).'_'.$realname{$link});
			$realname{$link} = $postnum{$link}.'_'.$realname{$link};	#should keep the files in sorted order, except for cases where the ten-thousands-place rolls around to 0. hmm
		}
		printf "%50s: ", $realname{$link}; say ($md5{$link}." :".(sprintf "%03d", $count).": ".$filesize{$link});
		
		$html =~ s/favicon-ws/favicon/;	#I just don't like the blue clover >_>
		
		$html =~ s/\d+\.thumbs/images/sig; $html =~ s/thumb/src/sig; $html =~ s/(\d+)s(?=\.jpg)/$1/sig;	#convert thumbnail elements to use full-size local images
		my $noidea = $link; $noidea =~ s/\w{3,4}$/jpg/;	
...
#			say "Warning: $ugh has hash $hashondisk{$ugh} on disk but $md5{$_} according to site";
	...
	print("Time since last image download: ".sprintf("%02d", int((time-$lastDL)/3600)).':'.sprintf("%02d", int((time-$lastDL)/60)%60).':'.sprintf("%02d", ((time-$lastDL)%60)).' :: ') unless $lastDL == 1;	#I'm sure there's a shorthand for most of this
	if ($count != (scalar(@files)-1)){ print ((($count - scalar(@files))+2).' known files left to download :: '); }	#something weird about negative numbers?
...
sub done {
	...
	my $there = $thread; $there =~ s{.+/(\w+)/res/(\w+)$}{$1/$2}; 
	$there =~ s[/.+$][/$threadtitle] unless $threadtitle eq '0';
	$here = $there;
	chdir $here;
	my @these = glob "*";
	chdir "..";
	$here =~ s{.+/(\w+)$}{$1}; $there =~ s{.+/(\w+)$}{$1};
	$there = '[DONE]'.$there;
	mkdir $there || die $!;
